...
func SaveBlockchain(nodeId string) error { //Blockları kaydeder
	Open(nodeId)
	defer Close()
	err := db.Update(func(tx *bolt.Tx) error {
...
func Last_Block_Index(nodeId string) int { //Listedeki tüm nesneleri dönderir
	Open(nodeId)
	defer Close()
...
func AllBlock() []Block {
	...
	defer Close()
	db.View(func(tx *bolt.Tx) error {
		c := tx.Bucket([]byte("3001.db")).Cursor()
		for k, v := c.First(); k != nil; k, v = c.Next() {
...
func DeleteBucket() {
	// delete file
	var err = os.Remove("C:/Users/Muhammed/Desktop/bitirme/blockchain-miner/blockchain-miner/3001.db")
	if isError(err) {
		return
	}
